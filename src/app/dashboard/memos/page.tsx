...
interface Memo {
    ...
    projectId: string;
    projectName?: string; // We'll need to fetch this or map it
}
    ...
    useEffect(() => {
        const fetchMemos = async () => {
            ...
            try {
                ...
                // Fetch projects to map project names if needed, or rely on future API update
                // For now, let's assume the API returns what we need or we mock the project name
                setMemos(data.map((m: any) => ({
                    ...m,
                    projectName: "Sample Project" // Placeholder until API provides joined project name
                })));
            } catch (error) {
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h2 className="text-3xl font-black tracking-tight text-[#0f172a] uppercase italic">
                        The Hub <span className="text-blue-600">/</span> Memos
                    </h2>
                    <p className="text-muted-foreground mt-1 text-sm font-bold uppercase tracking-widest opacity-70">
                        Micro-updates and critical project checkpoints
                    </p>
                </div>
                ...
                <CardHeader className="border-b border-slate-100 bg-slate-50/30 px-8 py-6">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="relative flex-1 max-w-md group">
                            ...
                            <Input 
                                placeholder="SEARCH MEMOS OR TEAM..." 
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <Badge className="bg-blue-50 text-blue-700 hover:bg-blue-50 border-none rounded-xl px-4 py-2 text-[10px] font-black uppercase tracking-[0.1em]">
                                {filteredMemos.length} TOTAL
                            </Badge>
                        </div>
                    </div>
                </CardHeader>
                                ...
                                <div key={memo.id} className="p-8 flex flex-col md:flex-row gap-8 hover:bg-slate-50/50 transition-all duration-300 group">
                                    ...
                                    <div className="flex-1 space-y-4">
                                        ...
                                        <div className="flex flex-wrap items-center gap-3 pt-2">
                                            ...
                                            <Link href={`/dashboard/projects/${memo.projectId}`} className="text-[9px] font-black text-blue-600 uppercase tracking-widest hover:underline flex items-center gap-1">
