    ...
    <>
      ...
      <div className="grid grid-cols-7 bg-slate-200 gap-px border-b border-slate-200">
        {calendarDays.map((day, idx) => (
          <div
            key={idx}
            className={`min-h-[140px] p-3 flex flex-col gap-2 transition-all duration-200 group relative ${
              day.isOtherMonth ? "bg-slate-50/50 opacity-40" : "bg-white hover:bg-slate-50"
              ...
              {!day.isOtherMonth && (day.isValidDate || day.hasMemo || day.hasEOD) && (
                <>
                      ...
                      className={cn(
                        ...
                        day.hasMemo 
                          ...
                          : day.isToday
                            ...
                            : "bg-red-50 text-red-700 border-red-200 hover:bg-red-100 opacity-60 hover:opacity-100"
                      )}
                      onClick={() => onDayClick(day, "memo")}
                    >
                      {day.hasMemo ? "✓ Memo" : day.isToday ? "Pend. Memo" : "No Memo"}
                    </Badge>
                  )}
                  ...
                  {(day.hasEOD || day.isValidDate) && (
                    ...
                    >
                      {day.hasEOD ? "✓ EOD" : day.isToday ? "Pend. EOD" : "No EOD"}
                    </Badge>
                  )}
                </>
              )}
